...
def print_hops(file, hops, times, mins, maxs):
    with open('%s' % (file), 'w') as f:
        f.write("hop\tips\ttime\tcant_ips\tmin_times\tmax_times\n")
        for i in hops.keys():
            if type(hops[i]) is list:
                f.write("%s\t[%s]" % (i, ",".join(hops[i])))
            else:
                f.write("%s\t%s" % (i, hops[i]))
            f.write("\t%.4f\t%s\t%.4f\t%.4f\n" % (times[i], str(len(hops[i])), mins[i], maxs[i]))
        f.closed
...
def main(i):
    ...
    for host in hosts.keys():
        ...
        hops, times, mins, maxs, zrtt = TR().send(host=host, packages=4)

        print_hops(arch, hops, times, mins, maxs)
